module.exports = function(connection,result,res,meetId){
    ...
    if(result[0].flag == 0){
        sqltwo = "select u.userImg as participants_img, m.fk_attendants_Id as participants_Id from meetAttendants as m join userImg as u on m.fk_attendants_Id = u.fk_userId where " +
        "m.fk_meet_Id = " + meetId +  ";";
        ...
        sqltwo = "select u.userImg as participants_img, m.fk_attendants_Id as participants_Id from endmeetAttendants as m join userImg as u on m.fk_attendants_Id = u.fk_userId where " +
        "m.fk_meet_Id = " + meetId +  ";";
        ...
        if(err) {res.status(400).json({"state" : 400}); console.log(err)}
            ...
            if(Object.keys(row).length > 0){
                for(var i = 0; i < Object.keys(row).length; i++){
                    ...
                    idArray.push(row[i].participants_Id);
                }
                
                result[0].participants_img = imgArray;
                result[0].participants_Id = idArray;
            }
