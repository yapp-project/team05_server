...
alter table meetendtable add CONSTRAINT `userIdid` FOREIGN KEY (`fk_meetcaptain`) REFERENCES `users` (`userId`) ON DELETE CASCADE ON UPDATE CASCADE;

alter table endmeetAttendants drop foreign key `fkattendantsId`;
alter table endmeetAttendants add CONSTRAINT `fkattendantsId` FOREIGN KEY (`fk_attendants_Id`) REFERENCES `users` (`userId`) ON DELETE CASCADE ON UPDATE CASCADE;

alter table noticecomment drop foreign key `fk_userId123` ;
alter table noticecomment add CONSTRAINT `fk_userId123` FOREIGN KEY (`fk_userId`) REFERENCES `users` (`userId`) ON DELETE CASCADE ON UPDATE CASCADE;
